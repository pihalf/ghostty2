...
pub const Options = struct {
    ...
    ///
    ///   ghostty2 +explain-config --option=font-size
    option: ?[]const u8 = null,

    /// The keybind action to explain. For example:
    ///
    ///   ghostty2 +explain-config --keybind=copy_to_clipboard
...
/// Examples:
///
///   ghostty2 +explain-config font-size
///   ghostty2 +explain-config copy_to_clipboard
///   ghostty2 +explain-config --option=font-size
///   ghostty2 +explain-config --keybind=copy_to_clipboard
///
/// Flags:
...
pub fn run(alloc: Allocator) !u8 {
    ...
    const name = keybind_name orelse option_name orelse positional orelse {
        ...
        try stderr_writer.interface.writeAll("Usage: ghostty2 +explain-config <option>\n");
        try stderr_writer.interface.writeAll("       ghostty2 +explain-config --option=<option>\n");
        try stderr_writer.interface.writeAll("       ghostty2 +explain-config --keybind=<action>\n");
