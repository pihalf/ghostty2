...
//
// `ghostty2 +new-window` is equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest io.github.pihalf.ghostty2 --object-path /io/github/pihalf/ghostty2 --method org.gtk.Actions.Activate new-window [] []
// ```
//
// `ghostty2 +new-window -e echo hello` would be equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest io.github.pihalf.ghostty2 --object-path /io/github/pihalf/ghostty2 --method org.gtk.Actions.Activate new-window-command '[<@as ["-e" "echo" "hello"]>]' []
// ```
