...
- Use `macos/build.nu` to build the macOS app, do not use `zig build`
  ...
  - Output: `macos/build/<configuration>/Ghostty2.app` (e.g. `macos/build/Debug/Ghostty2.app`)
...
- Test AppleScript support:
  ...
  (2) Launch and activate the app via osascript using the absolute path
      ...
      `osascript -e 'tell application "<absolute path to build/Debug/Ghostty2.app>" to activate'`
  ...
  (5) When done, quit via:
      `osascript -e 'tell application "<absolute path to build/Debug/Ghostty2.app>" to quit'`
